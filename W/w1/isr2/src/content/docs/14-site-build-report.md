...
<span class="chip">Baseline build cost $1.63</span>
</div>
...
| Site source hand-authored | **25 files · 256 KB · ~66k tokens** |
| Image/font binary assets produced | **10 files · 186 KB** (favicon, PWA icons, 1200×630 OG card) |
| Billable tokens (baseline scenario) | **4.39M in · 262k out · 4.65M total** |
| Direct compute cost on the actual model | **$1.63** — with 70% cached input: **$0.73** |
| Cheapest viable configuration | **$1.63** on DeepSeek V4.1 Flash (1.00× the actual model) |
| Most expensive configuration | **$57.01** on Claude Fable 5 (34.94×) |
| Spread, cheapest → priciest | **34.9×** |
| US vs China median | **$28.50** vs **$5.77** — a **4.9×** premium on US rate cards |
| Cost of not compacting context | **+$3.05** (2.9× the baseline) |

**The three numbers that matter:** a build of this size is a **$1.63** job on the model it actually ran on, a **$1.63–$57.01** job depending on which frontier model you point it at, and a **$6.49** job when the workload is tier-routed (cheap readers, mid-tier structurers, flagship synthesis) instead of run end-to-end on one expensive model. Every one of those variances is a *model selection* decision, not an engineering-effort decision.
...
| Hand-authored site source | 25 | 256 KB | ~66k | `scripts/`, `src/styles`, `src/components`, `src/integrations`, config |
...
flowchart LR
    ...
    G["Hand-authored tokens<br/>~66k"] --> H["× output overhead<br/>(reasoning, tool calls, diffs)"]
...
|---|---|---|---|---|---|---|
| **Lean (aggressive compaction)** | 6,000 | 2.66M | 171k | **2.83M** | $1.00 | $0.46 |
| **Baseline (moderate compaction)** ★ | 24,000 | 4.39M | 262k | **4.65M** | $1.63 | $0.73 |
| **Context-naive (no compaction)** | 130,000 | 14.56M | 262k | **14.83M** | $4.68 | $1.69 |
...
Each row applies that model's published input/output rates to the **baseline scenario** token counts (4.39M in / 262k out), sorted cheapest first. The relative index is measured against the model that actually ran the build (★ = 1.00×).

| # | Origin | Model | In / Out per 1M | Output share of cost | Build cost | vs actual |
|---|---|---|---|---|---|---|
| **★** 1 | China (actual build model) | **DeepSeek V4.1 Flash** | $0.30 / $1.20 | 19% | **$1.63** | 1.00× |
| 2 | China | **MiniMax M3** | $0.30 / $1.20 | 19% | **$1.63** | 1.00× |
| 3 | China | **DeepSeek V4 Pro 0813** | $0.43 / $0.87 | 11% | **$2.12** | 1.30× |
| 4 | United States | **Gemini 3.8 Flash** | $0.75 / $3.75 | 23% | **$4.28** | 2.62× |
| 5 | China | **Kimi K2.7 Code** | $0.95 / $4.00 | 20% | **$5.22** | 3.20× |
| 6 | China | **Kimi 2.6** | $0.95 / $4.00 | 20% | **$5.22** | 3.20× |
| 7 | China | **GLM-5** | $1.00 / $3.20 | 16% | **$5.23** | 3.20× |
| 8 | China | **GLM-5-Turbo** | $1.20 / $4.00 | 17% | **$6.32** | 3.87× |
| 9 | United States | **Muse Spark 1.2** | $1.25 / $4.25 | 17% | **$6.60** | 4.05× |
| 10 | China | **GLM-5.2** | $1.40 / $4.40 | 16% | **$7.30** | 4.47× |
| 11 | China | **Qwen3.8 Max** | $2.00 / $6.00 | 15% | **$10.35** | 6.34× |
| 12 | United States | **GPT-5.6 Terra** | $2.00 / $12.00 | 26% | **$11.93** | 7.31× |
| 13 | China | **Qwen3.7 Max** | $2.50 / $7.50 | 15% | **$12.94** | 7.93× |
| 14 | China | **Kimi K3** | $3.00 / $15.00 | 23% | **$17.10** | 10.48× |
| 15 | United States | **GPT-5.6 Sol** | $4.00 / $20.00 | 23% | **$22.80** | 13.98× |
| 16 | United States | **Claude Opus 5** | $5.00 / $25.00 | 23% | **$28.50** | 17.47× |
| 17 | United States | **Claude Opus 4.8** | $5.00 / $25.00 | 23% | **$28.50** | 17.47× |
| 18 | United States | **GPT-5.5** | $5.00 / $30.00 | 26% | **$29.82** | 18.28× |
| 19 | United States | **GPT-6 Astra** | $10.00 / $50.00 | 23% | **$57.01** | 34.94× |
| 20 | United States | **Claude Fable 5.1** | $10.00 / $50.00 | 23% | **$57.01** | 34.94× |
| 21 | United States | **Claude Fable 5** | $10.00 / $50.00 | 23% | **$57.01** | 34.94× |
...
xychart-beta
    ...
    y-axis "USD" 0 --> 60
    bar [1.63, 2.12, 5.23, 1.63, 17.10, 4.28, 22.80, 28.50, 29.82, 57.01]
```
...
|---|---|---|---|
| **Baseline** (uncached) | DeepSeek V4.1 Flash, no cache, on-peak | $1.63 | 1.00× |
| **KV-cache pinning** | 70% of input served from cache at $0.01/1M | $0.73 | 0.45× |
| **Off-peak batching** | DeepSeek off-peak window (50% off all rates) | $NaN | NaN× |
| **Tier-routed** | DeepSeek V4.1 Flash readers → Gemini 3.8 Flash / Qwen structurers → Claude Opus 5 for 30% of synthesis | $6.49 | 3.98× |
| **Frontier-only** | Every turn on Claude Fable 5.1 | $57.01 | 34.9× |
| **Context-naive** | DeepSeek V4.1 Flash, no compaction | $4.68 | 2.87× |
...
1. **Model selection moves the bill 35×; engineering effort does not.** The same repository, the same tokens and the same output cost $1.63 on DeepSeek V4.1 Flash and $57.01 on Claude Fable 5. Any cost-control programme that does not start with model routing is optimising the wrong variable.
...
4. **China rate cards remain 4.9× cheaper at the median** for this workload (US median $28.50 vs China median $5.77). The gap narrows as you move up the quality curve — Kimi K3 at 74.9 public score still undercuts every US model with a comparable score.
...
7. **Rebuilds are nearly free; re-reading the corpus is not.** The corpus read is 712,137 bytes of the 2,672,016-byte total surface. Because the pipeline is deterministic, a rebuild of this site consumes no research tokens at all — only the ~66k tokens of source it has to re-emit.
...
| Line item | Amount |
|---|---|
| Direct compute, this build (baseline scenario) | **$1.63** |
| Same build, cached + off-peak | **$NaN** |
| Same build on a US frontier flagship | **$57.01** (34.9× baseline) |
| Same build on the cheapest viable model | **$1.63** (1.00× baseline) |
| Annualised, one rebuild per working day | **$407.86** baseline · **$NaN** optimised |

**The asymmetry to budget for:** a single frontier-model build of this site costs $57.01; the same work costs $1.63 on a model that produces a materially similar artefact. Setting an explicit model policy — ban flagships from deterministic transformation, allow them only for final synthesis — is the difference between a $407.86 and a $14,251.63 annual line. The CFO-facing control is not "how much can we reduce token spend" but "which model is allowed to touch which step".
...
- **Where the leverage is.** The tier-routed configuration lands at $6.49 — 3.98× the single-model baseline — by keeping scouting on cheap models and reserving frontier synthesis for the 30% of output that needs it.
...
- **Strategic read.** The site you are reading cost **$1.63** in direct compute to produce, and the same work would have cost **$57.01** on the most expensive frontier configuration available today. The intelligence is no longer the scarce input — the *routing policy* is.
