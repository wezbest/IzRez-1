...
<span class="chip">Baseline build cost $1.48</span>
</div>
...
| Content emitted by the pipeline | **21 files · 1246 KB · ~319k tokens** (deterministic transforms, not token-generated) |
| Site source hand-authored | **22 files · 220 KB · ~56k tokens** |
| Image/font binary assets produced | **10 files · 186 KB** (favicon, PWA icons, 1200×630 OG card) |
| Billable tokens (baseline scenario) | **4.03M in · 225k out · 4.25M total** |
| Direct compute cost on the actual model | **$1.48** — with 70% cached input: **$0.65** |
| Cheapest viable configuration | **$1.48** on DeepSeek V4.1 Flash (1.00× the actual model) |
| Most expensive configuration | **$51.54** on Claude Fable 5 (34.85×) |
| Spread, cheapest → priciest | **34.9×** |
| US vs China median | **$25.77** vs **$5.24** — a **4.9×** premium on US rate cards |
| Cost of not compacting context | **+$3.05** (3.1× the baseline) |

**The three numbers that matter:** a build of this size is a **$1.48** job on the model it actually ran on, a **$1.48–$51.54** job depending on which frontier model you point it at, and a **$5.82** job when the workload is tier-routed (cheap readers, mid-tier structurers, flagship synthesis) instead of run end-to-end on one expensive model. Every one of those variances is a *model selection* decision, not an engineering-effort decision.
...
| Pipeline output (transformed) | 21 | 1246 KB | ~319k | `src/content/docs`, `src/data` |
| Hand-authored site source | 22 | 220 KB | ~56k | `scripts/`, `src/styles`, `src/components`, `src/integrations`, config |
...
flowchart LR
    ...
    C --> D["read tokens ≈ 1.73M"]
    D --> E["+ 96 turns × mean context re-sent"]
    E --> F["input tokens per scenario"]
    G["Hand-authored tokens<br/>~56k"] --> H["× output overhead<br/>(reasoning, tool calls, diffs)"]
...
|---|---|---|---|---|---|---|
| **Lean (aggressive compaction)** | 6,000 | 2.30M | 146k | **2.45M** | $0.87 | $0.39 |
| **Baseline (moderate compaction)** ★ | 24,000 | 4.03M | 225k | **4.25M** | $1.48 | $0.65 |
| **Context-naive (no compaction)** | 130,000 | 14.21M | 225k | **14.43M** | $4.53 | $1.61 |
...
Each row applies that model's published input/output rates to the **baseline scenario** token counts (4.03M in / 225k out), sorted cheapest first. The relative index is measured against the model that actually ran the build (★ = 1.00×).

| # | Origin | Model | In / Out per 1M | Output share of cost | Build cost | vs actual |
|---|---|---|---|---|---|---|
| **★** 1 | China (actual build model) | **DeepSeek V4.1 Flash** | $0.30 / $1.20 | 18% | **$1.48** | 1.00× |
| 2 | China | **MiniMax M3** | $0.30 / $1.20 | 18% | **$1.48** | 1.00× |
| 3 | China | **DeepSeek V4 Pro 0813** | $0.43 / $0.87 | 10% | **$1.93** | 1.30× |
| 4 | United States | **Gemini 3.8 Flash** | $0.75 / $3.75 | 22% | **$3.87** | 2.61× |
| 5 | China | **Kimi K2.7 Code** | $0.95 / $4.00 | 19% | **$4.73** | 3.20× |
| 6 | China | **Kimi 2.6** | $0.95 / $4.00 | 19% | **$4.73** | 3.20× |
| 7 | China | **GLM-5** | $1.00 / $3.20 | 15% | **$4.75** | 3.21× |
| 8 | China | **GLM-5-Turbo** | $1.20 / $4.00 | 16% | **$5.74** | 3.88× |
| 9 | United States | **Muse Spark 1.2** | $1.25 / $4.25 | 16% | **$5.99** | 4.05× |
| 10 | China | **GLM-5.2** | $1.40 / $4.40 | 15% | **$6.63** | 4.48× |
| 11 | China | **Qwen3.8 Max** | $2.00 / $6.00 | 14% | **$9.41** | 6.36× |
| 12 | United States | **GPT-5.6 Terra** | $2.00 / $12.00 | 25% | **$10.76** | 7.27× |
| 13 | China | **Qwen3.7 Max** | $2.50 / $7.50 | 14% | **$11.76** | 7.95× |
| 14 | China | **Kimi K3** | $3.00 / $15.00 | 22% | **$15.46** | 10.46× |
| 15 | United States | **GPT-5.6 Sol** | $4.00 / $20.00 | 22% | **$20.62** | 13.94× |
| 16 | United States | **Claude Opus 5** | $5.00 / $25.00 | 22% | **$25.77** | 17.43× |
| 17 | United States | **Claude Opus 4.8** | $5.00 / $25.00 | 22% | **$25.77** | 17.43× |
| 18 | United States | **GPT-5.5** | $5.00 / $30.00 | 25% | **$26.90** | 18.19× |
| 19 | United States | **GPT-6 Astra** | $10.00 / $50.00 | 22% | **$51.54** | 34.85× |
| 20 | United States | **Claude Fable 5.1** | $10.00 / $50.00 | 22% | **$51.54** | 34.85× |
| 21 | United States | **Claude Fable 5** | $10.00 / $50.00 | 22% | **$51.54** | 34.85× |
...
xychart-beta
    ...
    y-axis "USD" 0 --> 60
    bar [1.48, 1.93, 4.75, 1.48, 15.46, 3.87, 20.62, 25.77, 26.90, 51.54]
```
...
|---|---|---|---|
| **Baseline** (uncached) | DeepSeek V4.1 Flash, no cache, on-peak | $1.48 | 1.00× |
| **KV-cache pinning** | 70% of input served from cache at $0.01/1M | $0.65 | 0.44× |
| **Off-peak batching** | DeepSeek off-peak window (50% off all rates) | $NaN | NaN× |
| **Tier-routed** | DeepSeek V4.1 Flash readers → Gemini 3.8 Flash / Qwen structurers → Claude Opus 5 for 30% of synthesis | $5.82 | 3.94× |
| **Frontier-only** | Every turn on Claude Fable 5.1 | $51.54 | 34.9× |
| **Context-naive** | DeepSeek V4.1 Flash, no compaction | $4.53 | 3.06× |
...
1. **Model selection moves the bill 35×; engineering effort does not.** The same repository, the same tokens and the same output cost $1.48 on DeepSeek V4.1 Flash and $51.54 on Claude Fable 5. Any cost-control programme that does not start with model routing is optimising the wrong variable.
...
4. **China rate cards remain 4.9× cheaper at the median** for this workload (US median $25.77 vs China median $5.24). The gap narrows as you move up the quality curve — Kimi K3 at 74.9 public score still undercuts every US model with a comparable score.
...
6. **Caching is a 4–8× lever on input, not a rounding error.** With 70% of input served from cache the input side of this build collapses from $1.21 to $0.38. Static system prompts, the source registry and previously read files must be cache-pinned by construction.
7. **Rebuilds are nearly free; re-reading the corpus is not.** The corpus read is 712,137 bytes of the 2,212,718-byte total surface. Because the pipeline is deterministic, a rebuild of this site consumes no research tokens at all — only the ~56k tokens of source it has to re-emit.
...
| Line item | Amount |
|---|---|
| Direct compute, this build (baseline scenario) | **$1.48** |
| Same build, cached + off-peak | **$NaN** |
| Same build on a US frontier flagship | **$51.54** (34.9× baseline) |
| Same build on the cheapest viable model | **$1.48** (1.00× baseline) |
| Annualised, one rebuild per working day | **$369.70** baseline · **$NaN** optimised |

**The asymmetry to budget for:** a single frontier-model build of this site costs $51.54; the same work costs $1.48 on a model that produces a materially similar artefact. Setting an explicit model policy — ban flagships from deterministic transformation, allow them only for final synthesis — is the difference between a $369.70 and a $12,885.55 annual line. The CFO-facing control is not "how much can we reduce token spend" but "which model is allowed to touch which step".
...
- **Cost surfaces you control.** (1) Model routing per pipeline stage; (2) prompt-prefix stability, which decides whether the cache hit rate is $0.01/1M or $0.30/1M; (3) transcript compaction, worth 3.1× end-to-end; (4) off-peak batching at NaN×.
- **Where the leverage is.** The tier-routed configuration lands at $5.82 — 3.94× the single-model baseline — by keeping scouting on cheap models and reserving frontier synthesis for the 30% of output that needs it.
...
- **Strategic read.** The site you are reading cost **$1.48** in direct compute to produce, and the same work would have cost **$51.54** on the most expensive frontier configuration available today. The intelligence is no longer the scarce input — the *routing policy* is.
